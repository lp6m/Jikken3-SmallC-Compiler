     ...
     (lambda (bb i)
       (let* ((stmts (bblock-stmts bb))
         ...
         (cond ;; 基本ブロックの末尾の文で場合分けしながら，適切なエッジを張る
          ...
          ((ir-stx:write-stmt? last-stmt) (add-edge i (+ i 1)))
          ((ir-stx:read-stmt? last-stmt) (add-edge i (+ i 1)))
          ((ir-stx:call-stmt? last-stmt) (add-edge i (+ i 1)))
          ((ir-stx:print-stmt? last-stmt) (add-edge i (+ i 1)))
          ...
          (else (error "set-edges unknown stmt:" last-stmt)))))
     bbs idx)
    bbv))
